...
Prepare your environment:
  ...
  $ cp pybench.py test/
  $ cp test.sh test/
  $ cp LogServer.py test/
  $ cp -r gen-py test/
  $ cp process_log.py test/
  $ cp ../build64_release/pa_render_server.run test/
  $ cp ../build64_release/server/clientsim/RenderClientNewMain test/
  $ cp -r ../../message test/
  $ cp -r ../../model test/

  Now you can run the benchmark like this:
  $ python pybench.py -d ./model -s `pwd`/message -e "./RenderClientNewMain" -l  LogServer.py
  Also you can look more info about like this:
  $ python pybench.py -h

  or you can use test.sh for test
"""
...
def main():
    ...
    if options.run_times:
        if re.match("^\d+?(\.\d+?)?[dhms]$", options.run_times) is None:
            parser.print_help()
            sys.exit(1)
        run_secs = float(options.run_times[0:-1])
